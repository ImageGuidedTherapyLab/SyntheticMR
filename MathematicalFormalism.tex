...
\[
   p(z) = \int_\theta p(z|\theta) p (\theta) d \theta
        = \sum_q \omega_q p(z|\theta_q)
\]
A more direct calculation of the entropy is given as
\[
 MI = H(z)  = \int_z p(z) ln \; p(z) \; dz 
       \approx \int_z  \sum_q \omega_q p(z|\theta_q)  ln \; p(z) \; dz 
       \approx         \sum_{qr} \omega_{qr} \; ln \; p(z_r) 
       \approx         \sum_{qr} \omega_{qr} \; ln \; \sum_l \omega_l \; p(z_r|\theta_l)
\]
