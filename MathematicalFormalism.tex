...
\begin{equation}
\label{FFTmultiecho}
\mathcal{G}(k_x,k_y,k_z,\kk,\p) = \text{Fourier transform} \left\{ \underbrace{c_l(r) M_{TD}(\kk,\p,r) }_{\equiv f(r):\mathbb{R}^3 \rightarrow \mathbb{C} } \right\}
          = \mathcal{F}(f(r))
          = \int_\Omega \left(M_{TD}(\kk,\p,r)c_l(r)  \right)
...
\[
\begin{bmatrix}
z_r(k_x,k_y,k_z,\kk,\p) \\
z_i(k_x,k_y,k_z,\kk,\p) \\
\end{bmatrix}
= 
\begin{bmatrix}
\Re \left\{ \mathcal{G}(k_x,k_y,k_z,\kk,\p)  \right\} \\
\Im \left\{ \mathcal{G}(k_x,k_y,k_z,\kk,\p)  \right\} \\
\end{bmatrix}
 + 
\begin{bmatrix}
\nu_r \\
...
distribution. 
  \[ 
      p(\vec{z}|\p)   =  \mathcal{N}(\mathcal{G}(\kk;\p),\Sigma_z)  
  \]
...
Here $H(Z|\p)$ is constant by assumption of Gaussian noise.
