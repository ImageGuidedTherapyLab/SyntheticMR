...
function [kspace,MI,Hz]=MI_GH_QALAS(signal_lib,resized,mkfig)
...
% x2=x2(:);
% 
% [p1,p2,p3,p4,p5,p6]=ndgrid(flipAngle,TD(1,:),TD(2,:),TD(3,:),TD(4,:),TD(5,:));
% p1=p1(:);
% p2=p2(:);
% p3=p3(:);
% p4=p4(:);
...
parfor qp=1:lqp
    ...
    dt = [0,0,Tacq,0,0,p2(qp),0,p3(qp),0,p4(qp),0,p5(qp),0,p6(qp),0,0];
...
%     
%     for pp = 1:lpp
%         dt = [0,0,Tacq,0,0,p2(pp),0,p3(pp),0,p4(pp),0,p5(pp),0,p6(pp),0,0];
...
if resized==1
%     kspace=Mmodel;
    kspace(1,1,1,:,:,:,:,:,:,:,:)=reshape(Mmodel,[Msize(1),length(flipAngle),size(TD,2)*ones([1,size(TD,1)]),length(xm_t1)*length(xm_t2)]);
else
...
%     end
    ...
    lmm=length(Mmodel);
    parfor iii=1:lmm
        disp(sprintf('Fourier transform: %d of %d',iii,lmm))
        kspace(:,:,:,iii) = fftshift(fftn(Mmodel(iii)*materialID));
    end
    kspace=reshape(kspace,[size(materialID),Msize(1),length(flipAngle),size(TD,2)*ones([1,size(TD,1)]),length(xm_t1)*length(xm_t2)]);
end
